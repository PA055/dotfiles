...
if [[ $COMFYLINE_DATE_FORMAT == "" ]]; then
    COMFYLINE_DATE_FORMAT="%A, %B %e %Y"
fi

if [[ $COMFYLINE_TIME_FORMAT == "" ]]; then
    COMFYLINE_TIME_FORMAT="%l:%M:%S %p"
fi
...
segment_ranks=($RETVAL_RANK $HOST_RANK $USER_RANK $DIR_RANK $GIT_RANK $VENV_RANK $TIME_RANK $DATE_RANK)
